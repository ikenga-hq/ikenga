...
export function PermissionInbox({ sessionId }: { sessionId: string }) {
	...
	useEffect(() => {
		...
		listen<{ hook_event_name?: string; session_id?: string; tool_name?: string; tool_input?: Record<string, unknown>; prompt?: string }>(
			'hooks://event',
			(event) => {
	...
	const handleDecision = (id: string, decision: 'approved' | 'denied') => {
		...
		fetch('http://127.0.0.1:4000/iyke/hooks/decision', {
			method: 'POST',
	...
	return (
		<div className="flex h-full flex-col bg-zinc-950 p-3 text-xs font-mono text-zinc-200 select-none overflow-y-auto space-y-2">
			<div className="flex items-center gap-1.5 font-semibold text-amber-400 border-b border-border/40 pb-2">
				...
				<span>Permission Inbox ({requests.filter((r) => r.status === 'pending').length} pending)</span>
			</div>
			...
			{requests.map((req) => (
				...
				>
					<div className="flex items-center justify-between">
						...
						<span className="text-[10px] text-zinc-500">{new Date(req.timestamp).toLocaleTimeString()}</span>
					</div>
					...
					{req.status === 'pending' ? (
						...
						<div className="mt-1.5 text-[10px] font-semibold uppercase tracking-wider">
							Status: <span className={req.status === 'approved' ? 'text-emerald-400' : 'text-rose-400'}>{req.status}</span>
						</div>
					)}
				</div>
			))}
